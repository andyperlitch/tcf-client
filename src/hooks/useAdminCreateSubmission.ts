import {
  ...
  useAdminCreateSubmissionMutation,
...
export function useAdminCreateSubmission<T extends EngagementSubmissionData>() {
  ...
  const [createSubmissionMutation] = useAdminCreateSubmissionMutation({
    update: (cache, { data }) => {
      if (!data?.adminCreateSubmission) return;
      ...
      const existingData = cache.readQuery<AdminGetSubmissionsQuery>({
        ...
        variables: { engagementId: data.adminCreateSubmission.engagementId },
      });
          ...
          await Promise.all(
            presignedUrlResults.map(async (presignedUrlResult, index) => {
              ...
              if (
                ...
                !presignedUrlResult.data?.adminCreatePresignedUrl ||
